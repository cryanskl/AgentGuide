...
grep -q "uses: actions/checkout@" "$WORKFLOW_FILE"
grep -q "uses: actions/setup-node@" "$WORKFLOW_FILE"
grep -q "node-version: '22'" "$WORKFLOW_FILE"
grep -q "uses: actions/upload-pages-artifact@" "$WORKFLOW_FILE"
grep -q "uses: actions/configure-pages@" "$WORKFLOW_FILE"
grep -q "uses: actions/deploy-pages@" "$WORKFLOW_FILE"
